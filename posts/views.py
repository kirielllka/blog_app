...
class BasePermissionViewSet(ModelViewSet):
    ...
    def get_permissions(self):
        ...
        elif self.action in ('delete','destroy'):
            permission_classes = (IsAuthorOrReadOnly,)
        else:
            permission_classes = (IsAuthenticated,)
...
class ProfileViewSet(BasePermissionViewSet):
    ...
    def get_profile_by_user(self,request,pk):
...
class CommentViewSet(BasePermissionViewSet):
    queryset = Comments.objects.all().select_related('post','user')
    ...
    @action(detail=True, url_path='like', methods=['POST'])
    def like_comment(self, request, post_pk, pk):
